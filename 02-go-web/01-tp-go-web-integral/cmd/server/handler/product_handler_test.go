...
const pathFile = "./products_test.json"
...
func Test_Post_OK(t *testing.T) {
	...
	p, _ := loadProducts(pathFile)

	r.ServeHTTP(rr, req)
	actual := map[string]domain.Product{}
	_ = json.Unmarshal(rr.Body.Bytes(), &actual)
	_ = writeProducts(pathFile, p)
